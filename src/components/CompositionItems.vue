<template>
  <div class="border border-gray-200 p-3 mb-4 rounded">
    <div v-show="!showForm" class="flex justify-between">
      <div>
        <h4 class="text-2xl font-bold text-left">{{ song.modified_name }}</h4>
      </div>
...
<script>
import { songCollection, storage } from '@/includes/firbase'
export default {
  ...
  methods: {
    async edit(values) {
      ...
      try {
        ...
        this.alert_massage = error.message
        return
      }
    ...
    async deleteSong() {
      const storegRef = storage.ref()
      const songRef = storegRef.child(`songs/${this.song.original_name}`)
