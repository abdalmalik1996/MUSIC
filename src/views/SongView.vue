...
<script>
import { songCollection, auth, commentCollection } from '@/includes/firbase'
...
export default {
  ...
  async created() {
    const docSnapshots = await songCollection.doc(this.$route.params.id).get()
  ...
  methods: {
    ...
    async getComments() {
      const snapshots = await commentCollection.where('sid', '==', this.$route.params.id).get()
      this.comments = []
